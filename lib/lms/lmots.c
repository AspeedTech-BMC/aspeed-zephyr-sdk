...
#if (CONFIG_CRYPTO_ASPEED == 0)
    psa_hash_operation_t op = PSA_HASH_OPERATION_INIT;
#endif
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    size_t output_hash_len;
    unsigned short checksum;
#if CONFIG_CRYPTO_ASPEED
    struct hash_ctx ini;
    const struct device *dev = device_get_binding(HASH_DRV_NAME);
    struct hash_pkt pkt;
    pkt.out_buf = temp;
    pkt.out_buf_max = sizeof(temp);
#endif

#if CONFIG_CRYPTO_ASPEED
    status = hash_begin_session(dev, &ini, HASH_SHA256);
    has_engine_setup = 1;
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)params->I_key_identifier;
    pkt.in_len = MBEDTLS_LMOTS_I_KEY_ID_LEN;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)params->q_leaf_identifier;
    pkt.in_len = MBEDTLS_LMOTS_Q_LEAF_ID_LEN;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)D_MESSAGE_CONSTANT_BYTES;
    pkt.in_len = D_CONST_LEN;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)C_random_value;
    pkt.in_len = MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(params->type);
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    status = hash_final(&ini, &pkt);
    output_hash_len = ini.digest_size;
#else
                            ...
                            unsigned char *output)
{
    ...
    const struct device *dev = device_get_binding(HASH_DRV_NAME);
    struct hash_pkt pkt;
    pkt.out_buf = tmp_hash;
    pkt.out_buf_max = sizeof(tmp_hash);
#endif
    ...
    for (i_digit_idx = 0;
        ...
        for (j_hash_idx = j_hash_idx_min;
            ...
            mbedtls_lms_unsigned_int_to_network_bytes(i_digit_idx,
                                                      ...
                                                      i_digit_idx_bytes);
#if CONFIG_CRYPTO_ASPEED
            pkt.in_buf = (uint8_t *)i_digit_idx_bytes;
            pkt.in_len = I_DIGIT_IDX_LEN;
            status = hash_update(&ini, &pkt);
#else
            ...
            mbedtls_lms_unsigned_int_to_network_bytes(j_hash_idx,
                                                      ...
                                                      j_hash_idx_bytes);
#if CONFIG_CRYPTO_ASPEED
            pkt.in_buf = (uint8_t *)j_hash_idx_bytes;
            pkt.in_len = J_HASH_IDX_LEN;
            status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
            pkt.in_buf = (uint8_t *)tmp_hash;
            pkt.in_len = MBEDTLS_LMOTS_N_HASH_LEN(params->type);
            status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)D_PUBLIC_CONSTANT_BYTES;
    pkt.in_len = D_CONST_LEN;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)y_hashed_digits;
    pkt.in_len = MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(params->type) * MBEDTLS_LMOTS_N_HASH_LEN(params->type);
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_update(&ini, &pkt, ctx->params.I_key_identifier, sizeof(ctx->params.I_key_identifier));
        pkt.in_buf = (uint8_t *)ctx->params.I_key_identifier;
        pkt.in_len = sizeof(ctx->params.I_key_identifier);
        status = hash_update(&ini, &pkt);
#else
        status = psa_hash_update(&op,
                                 ctx->params.I_key_identifier,
...
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_update(&ini, &pkt, ctx->params.q_leaf_identifier, MBEDTLS_LMOTS_Q_LEAF_ID_LEN);
        pkt.in_buf = (uint8_t *)ctx->params.q_leaf_identifier;
        pkt.in_len = MBEDTLS_LMOTS_Q_LEAF_ID_LEN;
        status = hash_update(&ini, &pkt);
#else
        status = psa_hash_update(&op,
        ...
        mbedtls_lms_unsigned_int_to_network_bytes(i_digit_idx, I_DIGIT_IDX_LEN,
                                                  i_digit_idx_bytes);
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_update(&ini, &pkt, i_digit_idx_bytes, I_DIGIT_IDX_LEN);
        pkt.in_buf = i_digit_idx_bytes;
        pkt.in_len = I_DIGIT_IDX_LEN;
        status = hash_update(&ini, &pkt);
#else
        status = psa_hash_update(&op, i_digit_idx_bytes, I_DIGIT_IDX_LEN);
...
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_update(&ini, &pkt, const_bytes, sizeof(const_bytes));
        pkt.in_buf = const_bytes;
        pkt.in_len = sizeof(const_bytes);
        status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_update(&ini, &pkt, seed, seed_size);
        pkt.in_buf = seed;
        pkt.in_len = seed_size;
        status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
//        status = hw_sha256_finish(&ini, &pkt, temp, MBEDTLS_LMOTS_N_HASH_LEN_MAX, &output_hash_len);
        status = hash_final(&ini, &pkt);
        output_hash_len = ini.digest_size;
#else
        status = psa_hash_finish(&op,temp,
