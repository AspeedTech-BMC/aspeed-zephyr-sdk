...
/* Currently only support H=5 and H=10 */
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)r_node_idx_bytes;
    pkt.in_len = 4;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)D_LEAF_CONSTANT_BYTES;
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)pub_key;
    pkt.in_len = MBEDTLS_LMOTS_N_HASH_LEN(params->otstype);
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)D_INTR_CONSTANT_BYTES;
    pkt.in_len = D_CONST_LEN;
    status = hash_update(&ini, &pkt);
#else
...
#if CONFIG_CRYPTO_ASPEED
    pkt.in_buf = (uint8_t *)left_node;
    pkt.in_len = MBEDTLS_LMS_M_NODE_BYTES(params->type);
    status = hash_update(&ini, &pkt);
#else
                                  ...
                                  const unsigned char *key, size_t key_size)
{
    ...
    if (type != MBEDTLS_LMS_SHA256_M32_H10 &&
        type != MBEDTLS_LMS_SHA256_M24_H5) {
                       ...
                       const unsigned char *sig, size_t sig_size)
{
    ...
    if( ots_type != MBEDTLS_LMS_SHA256_M32_H10 &&
        ots_type != MBEDTLS_LMS_SHA256_M24_H5 )
    {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
                                     ...
                                     size_t seed_size)
{
    ...
    if (ctx->ots_private_keys == NULL) {
        printf("try to allocate %d * %d for private key\n", MERKLE_TREE_LEAF_NODE_AM(ctx->params.type), sizeof(*ctx->ots_private_keys));
    ...
    if (ctx->ots_public_keys == NULL) {
        printf("try to allocate %d * %d for public key\n", MERKLE_TREE_LEAF_NODE_AM(ctx->params.type), sizeof(*ctx->ots_public_keys));
        ret = MBEDTLS_ERR_LMS_ALLOC_FAILED;
        goto exit;
    }
                     ...
                     size_t *sig_len)
{
    ...
    if (ctx->params.type != MBEDTLS_LMS_SHA256_M32_H10 &&
        ctx->params.type != MBEDTLS_LMS_SHA256_M24_H5) {
        return MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
